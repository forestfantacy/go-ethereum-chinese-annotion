...
/**
协商过程就是认证的过程：输入当前节点私钥，输出对端节点公钥，
	发起端发送认证请求，等待认证回执
	接收端接收认证请求，验证签名，发送认证回执
	handshakeState 提供双方的读写能力和各种秘钥
*/
...
处理认证请求：
	...
	获取远端公钥、随机数
	使用本端私钥和远端公钥生成共享秘钥token
	使用token还原远端随机数的签名明文
	还原远端随机公钥，如果成功，则认为认证通过
*/
...
/**
发起者发起认证过程
  认证请求 加密（随机数 & 签名 & 公钥）-》对端
  认证回执 把随机数、公钥记录到 handshakeState
*/
