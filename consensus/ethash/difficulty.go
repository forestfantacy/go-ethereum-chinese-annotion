...
// 难度调整算法。它返回在给定父块的时间和难度的情况下创建新块时应该具有的难度。计算使用Frontier规则
...
// 难度调整算法。它返回在给定父块的时间和难度的情况下创建新块时应该具有的难度。计算使用Homestead规则
...
// 使用给定的bomb-delay创建一个difficultyCalculator。难度是用拜占庭规则计算的，不同于Homestead的是叔块如何影响计算
